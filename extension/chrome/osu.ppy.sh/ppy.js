...
function createNotifications(data){
	...
	notificationPopupItem.classList.add('notification-popup-item', 'notification-popup-item--group', 'notification-popup-item--user_achievement_unlock');
	...
	categoryRow.classList.add('notification-popup-item__row', 'notification-popup-item__row--category');
	categoryRow.textContent = data.title;// *** //
	content.appendChild(categoryRow);
	const messageRow = document.createElement('div');
	messageRow.classList.add('notification-popup-item__row', 'notification-popup-item__row--message');
	messageRow.textContent = data.subtitle;// *** //
	content.appendChild(messageRow);
	const timeRow = document.createElement('a');
	timeRow.classList.add('notification-popup-item__row', 'notification-popup-item__row--time', 'u-hover');
	timeRow.href = '{url2}';// *** //
	...
	actionButton.classList.add('notification-action-button', 'notification-action-button--fancy');
	...
	document.querySelector("#notification-widget > div > div > div.notification-stacks").appendChild(notificationPopupItem)
    actionButton.addEventListener('click', (event)=> {
        notificationPopupItem.remove()
    })
    // document.querySelector("#notification-widget > div > div > div.notification-popup__buttons > div > button").addEventListener('click', (event)=>{
    //     notificationPopupItem.remove()
    // })
}
