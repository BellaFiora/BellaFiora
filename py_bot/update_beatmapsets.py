...
def _update_beatmapsets(start:int, end:int, apikey:str, nb_beatmapsets_downloaded:int, lock:multiprocessing.Lock) -> None:
...
def update_beatmapsets() -> None:
	while True:
		...
		for k, (i, j) in enumerate(zip(ranges)):
			p = Process(target=_update_beatmapsets, args=(i, j, apikeys[k], nb_beatmapsets_downloaded, lock,))
			p.start()
