...
async function handleRequest(query){
  ...
  if(versionNumber < minimalVersion){
    ...
    return `Please update bellafiora. Minimum version accepted: ${inimalVersion}`
    ...
    console.log(`Client ${query.idApp}: is logged, on ${query.ipAddress} and use version ${query.version}: Lasted Softare version, MD5 is Ok.`)
    console.log(`Client ${query.idApp}: OS: ${query.platform} /${query.arch} | ${query.memory} RAM | ${query.os_release}`)
