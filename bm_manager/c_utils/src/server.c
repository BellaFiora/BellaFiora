...
__attribute__ ((constructor)) static void setup_servers(void) {
	...
	if (!servers)
		ereport("calloc failed: %s", strerror(errno));
}
...
Server* new_server(char* name, char* port, void* (*callback)(void*), void* (*sigint_handler)(void*)) {
	...
	if ((status = getaddrinfo(NULL, port, &hints, &res))) {
		ereport("getaddrinfo failed: %s", gai_strerror(status));
		return NULL;
	}
	...
	for (p = res; p; p = p->ai_next) {
		if ((server_socket = socket(p->ai_family, p->ai_socktype, p->ai_protocol)) == -1) {
			report("socket failed: %s", strerror(errno));
			continue;
		}
		...
		if (setsockopt(server_socket, SOL_SOCKET, SO_REUSEADDR, &optval, sizeof(optval)) == -1)
			ereport("setsockopt failed: %s", strerror(errno));

		if (bind(server_socket, p->ai_addr, p->ai_addrlen) == -1) {
			report("bind failed: %s", strerror(errno));
			close(server_socket);
			continue;
		}
		break;
	...
	freeaddrinfo(res);
	if (!p) {
		ereport("no suitable address was found: %s", strerror(errno));
		return NULL;
	}

	// Listen for incoming connections
	if (listen(server_socket, 10) == -1) {
		ereport("listen failed: %s", strerror(errno));
		close(server_socket);
		return NULL;
	}
	...
	if (!r) {
		ereport("malloc failed: %s", strerror(errno));
		close(server_socket);
		return NULL;
	}
	r->name = name;
...
int start_server(Server* server) {
	if (pthread_create(&server->th, NULL, server->callback, (void*)server)) {
		ereport("pthread_create failed: %s", strerror(errno));
		free(server);
		return 1;
	}
	return 0;
...
int wait_server(Server* server) {
	if (pthread_join(server->th, NULL)) {
		ereport("pthread_join failed: %s", strerror(errno));
		free(server);
		return 1;
	}
	return 0;
...
int setup_sigint_handlers(void) {
	if (signal(SIGINT, sigint_handlers) == SIG_ERR) {
		ereport("signal failed: %s", strerror(errno));
		return 1;
	}
	return 0;
}
