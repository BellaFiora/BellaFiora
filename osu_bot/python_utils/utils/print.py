...
def cprint(string, color=37, end="\n"):
	if type(color) is str:
		color = color_codes.get(color.lower())
	if color:
...
def print_var(var, indent=3, color=37):
	...
	cprint(
		pprint.pformat(
			str([k for k, v in callers_local_vars if v is var][0]) + " = " + str(var),
			indent=indent,
		),
		color=color,
	)
...
def _print_json(obj, indent, color):
	if type(obj) is str or type(obj) is bytes:
		try:
			...
			obj = str(obj)
			cprint(
				f"print_json: json.loads() failed loading the provided string or bytes ({obj})"
			)
			return
...
def print_response(r, indent=3, color=37, content_color=37):
	if color != 37 and content_color == 37:
		content_color = color
	...
	if r._content != b"":
		...
		if "Content-Encoding" in headers:
			...
			content_encoding_in_headers = False

		if content_encoding in headers:
			if (
				"gzip" in headers[content_encoding]
				or "compress" in headers[content_encoding]
				or "deflate" in headers[content_encoding]
			):
		...
		if content_type_in_headers:
			if "text/plain" in headers[content_type]:
				...
				cprint(
					BeautifulSoup(content, "html.parser").prettify(indent_width=indent),
					color=content_color,
				)
			elif "application/json" in headers[content_type]:
				cprint(
					json.dumps(json.loads(content), indent=indent), color=content_color
				)
