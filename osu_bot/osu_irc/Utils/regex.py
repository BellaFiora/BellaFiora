...
ReModeInfo: "re.Pattern" = re.compile(
	r"^:.+?!cho\@cho.ppy\.sh MODE (#?\S+?) (\+|-)(o|v) (\S+)$"
)
...
# custom
ReAll: "re.Pattern" = re.compile(r"^:(.*)!cho@ppy.sh (.*) :(.*)\n$")
ReActionNp: "re.Pattern" = re.compile(
	r"^\x01ACTION is listening to \[([^ ]*) (.*) - (.*)\]\x01$"
)
