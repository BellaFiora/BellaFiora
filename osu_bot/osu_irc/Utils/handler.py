...
from ..Utils.regex import ReUserListData, ReQuit, ReMOTDInfo, ReModeInfo
...
async def handleJoin(cls: "Client", payload: str) -> bool:
	...
	Log.debug(
		f"Client launching: Client.onMemberJoin: {str(vars(Chan))} {str(vars(KnownUser))}"
	)
...
async def handlePart(cls: "Client", payload: str) -> bool:
	...
	if PartUser.name.lower() == cls.nickname.lower():
		...
		Log.debug(
			f"Client parted a channel, removing {PartUser._generated_via_channel}"
		)
	...
	Log.debug(
		f"Client launching: Client.onMemberPart: {str(vars(Chan))} {str(vars(KnownUser))}"
	)
...
async def handleQuit(cls: "Client", payload: str) -> bool:
	...
	Log.debug(
		f"Client launching: Client.onMemberQuit: {str(vars(QuitingUser))} {reason}"
	)
...
async def handleUserList(cls: "Client", payload: str) -> bool:
	...
	if search:
		...
		for user_name in full_user_list.split(" "):
			...
			if user_name.lower() in ["", " ", cls.nickname.lower()]:
				continue
			...
			Log.debug(
				f"New Entry to `already-known-user-list`: {ChannelToFill.name} - {KnownUser.name}"
			)
...
async def handlePrivMessage(cls: "Client", payload: str) -> bool:
	...
	if Author:
		...
		Log.debug(
			f"Client launching: Client.onMemberJoin: {str(vars(Chan))} {str(vars(Alternative))}"
		)
