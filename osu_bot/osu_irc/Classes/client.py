	...
	* `request_limit` int : (Default: 15)[ How many requests can be send before the client goes into rate limit protection (request_limit per 60 sec)]
	...
	async def main(self) -> None:
		...
		while self.running:
			...
			try:
				# init connection
				self.ConnectionReader, self.ConnectionWriter = (
				...
				Log.error(
					"Invalid Auth for osu!, please check `token` and `nickname`, not trying to reconnect"
				)
				self.stop()
				continue
				...
				Log.error(
					"osu! never send any response, check credentials for syntax, not trying to reconnect"
				)
				self.stop()
				continue
	...
	async def sendContent(
		self, content: bytes or str, ignore_limit: bool = False
	) -> None:
		"""
		...
		if (self.traffic <= self.request_limit) or ignore_limit:
			...
			Log.debug(
				f"Client sending {len(content)} bytes of content to the ConnectionWriter"
			)
	...
	async def sendMessage(self, Chan: Union[Channel, str], content: str):
		...
		if isinstance(Chan, Channel):
			...
			raise ValueError(
				f"sendMessage() is meant for channels only, please use sendPM() for messages to a user"
			)
		else:
	...
	async def sendPM(self, Us: Union[User, str], content: str):
		...
		if isinstance(Us, User):
			...
			raise ValueError(
				f"sendPM() is meant for users only, please use sendMessage() for messages to a channel"
			)
		else:
