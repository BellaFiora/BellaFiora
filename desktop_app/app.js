...
// const { Beatmap, Osu: { DifficultyCalculator, PerformanceCalculator} } =
// require('osu-bpdpc') const dns = require('dns'); const { type } =
// require('os');
...
app.whenReady().then(async () => {
	const Conf = new conf();
	Conf.setConf(
		'AppPath', app.getAppPath().replace('\\resources\\app.asar', ''));
	// fs.readFile(path.resolve(Conf.getConf('AppPath'),'./package.json'),
	// 'utf8', (err, output) => {
...
async function osuConnect() {
		...
		if (Conf.getConf('osu_token')) {
			...
			axios
				.get('https://osu.ppy.sh/api/v2/me', {
					headers : {
						'Authorization' : `Bearer ${Conf.getConf('osu_token')}`
					},
				})
				.then(async response => {
	...
	async function AuthService() {
		return new Promise(async (resolve, reject) => {
			...
			shell.openExternal(
				'https://osu.ppy.sh/oauth/authorize?client_id=30165&redirect_uri=https://techalchemy.fr/oAuth2/Bellafiora/index.php&response_type=code&scope=public identify');
	...
	async function SyncData() {
		...
		return new Promise(async (resolve, reject) => {
			try {
				...
				await RemoteSrv
					.sync(
						Conf.getConf('client_id'), Conf.getConf('osu_token'),
						Conf.getConf('osu_id'))
					.then(resp => {
						if (!resp) {
...
async function LaunchGUI() {
	if (!continueExecute)
		return return new Promise(async (e, n) => {
			const filePath = path.join('./app/src/html/body.raw');
			const rawHTML = fs.readFileSync(filePath, 'utf-8');
			const artisan = new Artisan();
			let default_mode = player_data.gameplay.playmode
			default_mode = (default_mode === 'osu') ? 'm0' : (default_mode === 'mania') ? 'm3' : (default_mode === 'taiko') ? 'm1' : (default_mode === 'fruits') ? 'm2' : 'm0';
			let gameplay = player_data.gameplay[default_mode]
				...
				= {
					  ...
					  .create({
						  props : {
							  lang : (player_data.basic_informations.country)
										 .toLowerCase(),
							  // lang: 'en',
							  objectType : 'document',
							  format : 'html',
							  ...
							  ressources : {
								  css : [
									  '../src/style.css',
									  'https://cdnjs.cloudflare.com/ajax/libs/rangeslider.js/2.3.2/rangeslider.css'
								  ],
								  scripts : [
									  {
								  ...
								  links : [
									  ...
									  {
										  href :
											  'https://fonts.googleapis.com/css2?family=Noto+Sans+Thai:wght@300&display=swap',
										  rel : 'stylesheet',
										  position : 'Header'
									  },
									  {
...
async function gLaunch() {
	if (!continueExecute)
		return Log('Starting Gosumemory');
	return new Promise(async (o, r) => {
		let s;
		try {
			const Conf = new conf();
			fs.access(
				path.join(Conf.getConf('AppPath'), 'gosumemory.exe'),
				fs.constants.F_OK, e => {
					if (e) {
						Log(`Error trying to launch Gosumemory:<br>${e}`);
						o(false)
					} else {
						externalProcess = spawn(path.join(
							Conf.getConf('AppPath'), 'gosumemory.exe'));
							...
							if (externalProcess) {
								externalProcess.stdout.on('data', async e => {
									...
									if (e.includes(
										...
										|| e.includes('Initialization complete')) {
										gmIsReady = true;
										clearTimeout(s);
										const gIni = new gini();
						...
						const t = new Promise(n => {
							s = setTimeout(() => {
								if (!gmIsReady) {
									if (isOsuLanuched) {
										Log('If this is the first time, start Osu!',
											true, true);
										setTimeout(async () => {
											o(false);
											isOsuLanuched = false;
										...
										Log('Error When Launching Gosumemory',
											true, true);
										o(false)
									};
								} else {
						...
						Promise.race([ n, t ]).then(e => { o(e) }).catch(e => {
			...
			Log(`Error When Launching Gosumemory: ${e.message}`, true, true);
			o(e)
		};
	});
};
		...
		return
			...
			return new Promise(async (resolve, reject) => {
				...
				if (fs.existsSync(
					...
					// Parse osu database and scan beatmaps using osuFiles
					// object
					OsuFiles.osuDbParse(mainWindow).then(() => {
						// Log message for scanning scores
						Log('Scan your scores', true, false);

						// Parse score database and update scores using osuFiles
						// object
...
async function Log(e, show = true, err = false, toServ = true) {
	...
	if (show) {
		if (err) {
			try {
				await mainWindow.webContents.send('loading_err', e)
			} catch (e) {
				console.log(e);
			};
		} else {
