!function(a, b) {
	...
	function a(b, c, d) {
		...
		this.url = b, this.reconnectAttempts = 0,
		this.readyState = WebSocket.CONNECTING, this.protocol = null;
			...
			this.open = function(b) {
				...
				h.onopen = function() {
					clearTimeout(e),
						...
						g.protocol = h.protocol, g.readyState = WebSocket.OPEN,
					...
					if (clearTimeout(e), h = null, i)
						...
						var d = l('connecting');
						d.code = c.code, d.reason = c.reason,
						d.wasClean = c.wasClean, k.dispatchEvent(d),
				...
				if (h)
					return (g.debug || a.debugAll) && console.debug('ReconnectingWebSocket', 'send', g.url, b),
						   h.send(b);
	...
	return a.prototype.onopen = function() {},
		   ...
		   a.prototype.onerror = function() {}, a.debugAll = !1,
		   a.CONNECTING = WebSocket.CONNECTING, a.OPEN = WebSocket.OPEN,
		   a.CLOSING = WebSocket.CLOSING, a.CLOSED = WebSocket.CLOSED, a
});