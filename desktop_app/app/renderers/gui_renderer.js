...
async function IntroduceDataPlayer(defaultMod = false) {
  ...
  if(!defaultMod){
    defaultMod = (basic_infos.playmode === 'osu') ? 0 :
      (basic_infos.playmode === 'mania') ? 3 :
      (basic_infos.playmode === 'ctb') ? 2:
      (basic_infos.playmode === 'taiko') ? 1 :
      '?';
  } else {
    defaultMod = (defaultMod === 'osu') ? 0 :
      (defaultMod === 'mania') ? 3 :
      (defaultMod === 'ctb') ? 2:
      (defaultMod === 'taiko') ? 1 :
      '?';
...
function playVoice(language, type) {
  console.log(audioCache);
      ...
      if (filteredResults.length === 0) {
          console.error('Aucun résultat trouvé pour la langue et le type spécifiés.');
          return;
      }
      function rd(){
      ...
      function play(randomVoiceInfo) {
        ...
        audioContext.decodeAudioData(rdBuffer.buffer, (audioBuffer) => {
      ...
      console.error('Le cache audio est vide. Chargez d\'abord les voix.');
  }
}
...
setTimeout(() => {
  setInterval(async () => {
    // if()
    updateProgressBar(wsData.menu.bm.time.current, wsData.menu.bm.time.full);
    document.getElementById('timeElapsed').innerHTML = `
    <span>${ctm(wsData.menu.bm.time.current, wsData.menu.bm.time.full)}</span>-<span>${ctm(wsData.menu.bm.time.full, wsData.menu.bm.time.full)}</span>`
    document.getElementById('MusictimeElapsed').innerHTML=`
    <span>${ctm(wsData.menu.bm.time.current, wsData.menu.bm.time.mp3)}</span>-<span>${ctm(wsData.menu.bm.time.mp3, wsData.menu.bm.time.mp3)}</span>
    `
