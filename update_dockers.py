...
docker_manager = DockerManager(
	*args, dotenv_path=root + "common/env/.env", root_depth=root_depth
)
...
def process_commit(commit):
	...
	commit_hash = tmp[0]
	if commit_hash in commits_done:
		return
	edits = tmp[2:-1]
	for edit in edits:
		tmp = edit.split("\t")
...
time_other = (
	time_total - time_spent_on_ssh - time_measuring_on_ssh - time_to_restart_dockers
)
print(
