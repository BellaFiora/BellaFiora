...
app.get('/private/*', requestHandler);
app.get('/public/*', requestHandler);
...
async function requestHandler(req, res) {
	...
	try {
		...
		if (endpoint) {
			...
			try {
				...
				if (responseData.error && responseData.error === 'Unauthorized') {
					...
					res.status(responseData.status_code || 200)
						.json(responseData.error);
				} else {
					res.status(responseData.status_code || 200)
						.json(responseData);
					logs = {
		...
		await entryMetrics(
			'public_api', startTime, elapsedTime, exitCode,
			JSON.stringify(logs));
	} catch (err) {
		logs = {
			client : null,
...
// string: [status] (liste séparée par des virgules) | Accepcté: graveyard ou/et
// ranked ou/et loved ou/et qualified ou/et approved int: max_sort | accepté:
// entre 1 et 1000 string: order_by | accepté: asc ou desc
