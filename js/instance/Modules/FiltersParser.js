        ...
        if (option) {
          ...
          if (option.arguments > 0 && argumentIndex < inputArray.length) {
            ...
            if (option.format === "string" && option.acceptedArgument.length > 0 && !option.acceptedArgument.includes(argument)) {
              this.errors.push({ error: 6, error_name: `Invalid argument for --${option.option}: ${argument}` });
              argument = null;
            }
          } else if (option.arguments > 0) {
            this.errors.push({ error: 3, error_name: `Missing argument for --${option.option}` });
          }
          ...
          if (option.min_value !== null && option.max_value !== null) {
            if (argument < option.min_value || argument > option.max_value) {
              this.errors.push({ error: 4, error_name: `Value for --${option.option} is out of range [${option.min_value}, ${option.max_value}]` });
            } else {
