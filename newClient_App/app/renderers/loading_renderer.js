...
ipcRenderer.on('progress_loading', (event, stat) => {
    ...
    document.getElementById('progress_loading').insertAdjacentHTML('beforeend', MessagEvent)
})
...
ipcRenderer.on('ws_connect', (event) => {
    ...
    socket.onerror = error => {
        const lastElm = document.getElementById('progress_loading').lastElementChild
        lastElm.innerHTML = `Connection to the Websocket ❌`
        ipcRenderer.send('ws_connect_result', 'Unable to connect to websocket. Please check Bella Fiora installation or If Osu! Is well started')
        console.log(error)
       
    };
})
    
ipcRenderer.on('loading_err', (event, error) => {
    document.getElementById('loading_error').innerText = `${error}`
})


ipcRenderer.on('client_informations', (event, infos)=>{
    document.getElementById('client_informations').innerText = `v${infos.client_version}`
})
