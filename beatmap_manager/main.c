...
#define creport(fd, msg, ...)                                                  \
	freport(fd, "%s: " msg, server->name, ##__VA_ARGS__)
...
void* callback(void* data) {
	...
	*/
	...
	timeout.tv_sec = 1;
	timeout.tv_usec = 0;
	if (setsockopt(server->socket, SOL_SOCKET, SO_RCVTIMEO, &timeout,
				   sizeof(timeout))
		== -1)
	...
	while (server->running) {
		...
		*/
		...
		int client_socket =
			accept(server->socket, &client_addr, &client_addr_len);
		if (client_socket == -1) {
			// Error or timeout
		...
		bytes_received = recvfrom(client_socket, buf, BUF_SIZE, 0, &client_addr,
								  &client_addr_len);
		...
		// int s = getnameinfo(&client_addr, client_addr_len, host, NI_MAXHOST,
		// service, NI_MAXSERV, NI_NUMERICSERV); if (s)
		//  fprintf(stderr, "%s: getnameinfo failed: %s\n", server->name,
		//  gai_strerror(s));
		// else
		//  printf("%s: received %ld bytes from %s:%s\n", server->name, (long)
		//  bytes_received, host, service);
		...
		if (sendto(client_socket, hello, bytes_to_send, 0, &client_addr,
				   client_addr_len)
			!= bytes_to_send)
...
int main(void) {
	...
	Server* server1 =
		new_server(server1_name, "25586", callback, sigint_handler);
	if (!server1) {
	...
	Server* server2 =
		new_server(server2_name, "25587", callback, sigint_handler);
	if (!server2) {
