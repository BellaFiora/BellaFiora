	...
	while (1) {
		...
		if ((file = fopen(buffer, "wb+"))) {
			if (fwrite(curl_response.data, curl_response.size, 1, file))
				...
				fprintf(stderr,
						"download_beatmaps: fwrite failed to write to %s.osu\n",
						bmid);
			fclose(file);
		} else
			fprintf(stderr,
					"download_beatmaps: fopen failed to create %s.osu\n", bmid);
		sleep(1);
	}
	return NULL;
}
	...
	do {
		...
		json_array_foreach(bmset, index, bm) {
			if (!(tmp = json_object_get(bm, "beatmap_id"))) {
				fprintf(
					stderr,
					"scan_beatmapsets: no beatmap_id in %zuth beatmap of %s\n",
					index, entry->d_name);
				continue;
			}
			...
			if (!bmid) {
				fprintf(stderr,
						"scan_beatmapsets: null beatmap_id in %zuth beatmap of "
						"%s\n",
						index, entry->d_name);
				continue;
			}
...
int update_beatmaps(void) {
	...
	update_beatmaps_check(
		pthread_create(&dl_thread, NULL, download_beatmaps, q),
		"pthread_create failed");
	update_beatmaps_check(
		pthread_create(&scan_thread, NULL, scan_beatmapsets, q),
		"pthread_create failed");
	printf("updating beatmaps...\n");
	int st = time(NULL);
	update_beatmaps_check(pthread_join(scan_thread, NULL),
						  "pthread_join failed");
